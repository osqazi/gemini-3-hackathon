#!/usr/bin/env python3
"""
Script to populate recipe_cards table with recipes from recipes table
...
def populate_recipe_cards():
    """
    Populate recipe_cards table with recipes from recipes table
    """
    logger.info("Starting recipe_cards population process...")
    ...
    try:
        with db_manager.get_connection() as conn:
            ...
            for row in recipe_rows:
                ...
                cook_time = prep_time  # Use prep_time as cook_time if not available
                total_time = prep_time if prep_time else None
                ...
                values = (
                    ...
                    json.dumps([category] if category else []),  # tags (array with category)
            ...
            logger.info("Setting public=True, user_id=user_8447729dea2b3059, and generated_at to today")

    except Exception as e:
        logger.error(f"Error populating recipe_cards table: {e}")
        raise

def main():
    """
    Main function to execute the recipe_cards population process
    """
    logger.info("Starting recipe_cards population process...")

    try:
        populate_recipe_cards()
        logger.info("Recipe cards population completed successfully!")
        return 0
    except Exception as e:
        logger.error(f"Recipe cards population failed: {e}")
        return 1
